...
there are so many different between  getElementById, getElementsByClassName, and querySelector / querySelectorAll but main difference is selector ability.  
getElementById for only one id. 
getElementsByClassName for only class.
querySelector  returns the first element that matches the specified selectors. 
And querySelectorAll returns all matching elements as a NodeList
